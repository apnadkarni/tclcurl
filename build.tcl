...
proc _install {args} {
	...
	if {[string length $openssl]} {
	    lappend rcargs -L [file join $openssl lib] -I [file join $openssl include]
	} else {
	    lappend rcargs -L $ldir
	}
...
proc Hdebug {} { return "?destination? ?config? ?-curl <path>? ?-openssl <path>? ?-static? ?-dynamic?\n\tInstall debug builds of all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-curl <path> = path to Curl\n\t-openssl <path> = path to OpenSSL\n\t-static = link statically, default is to link dynamically\n\t-dynamic = link dynamically" }
proc _debug {args} {
    global packages
    set d [eval configure $args]
    dict with d {}
	...
	lappend rcargs -keep -debug all -cache [pwd]/BUILD.$p -I [pwd]
	if {[string length $curl]} {
	    lappend rcargs -L [file join $curl lib] -I [file join $curl include]
	} else {
	    lappend rcargs -L $ldir
...
proc _gui {} {
    global INSTALLPATH CURLPATH OPENSSLPATH BUILDSTATIC
    package require Tk
#    package require widget::scrolledwindow
    ...
    label  .lz -text {Curl Path: }
    entry  .ez -textvariable ::CURLPATH
    button .bz -command [list Browse ::CURLPATH] -text Browse
    label  .lo -text {Openssl Path: }
    entry  .eo -textvariable ::OPENSSLPATH
    button .bo -command [list Browse ::OPENSSLPATH] -text Browse
    checkbutton .bs -text "Link statically" -variable BUILDSTATIC -anchor w
...
proc Install {} {
    global INSTALLPATH CURLPATH OPENSSLPATH NOTE
	...
	_install $INSTALLPATH -curl $CURLPATH -openssl $OPENSSLPATH %s
...
proc Debug {} {
    global INSTALLPATH CURLPATH OPENSSLPATH
    foreach p {.ei .ez .bi .bz .bs .i .d} {
	$p configure -state disabled
    }

    set fail [catch [format {
	_debug $INSTALLPATH -curl $CURLPATH -openssl $OPENSSLPATH %s
